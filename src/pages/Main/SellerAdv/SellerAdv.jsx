...
export default function SellerAdv() {
  ...
  const advVisibilityNumber = useSelector(
    (state) => state.ads.numberVisibility,
  );
  const activeImage = useSelector((state) => state.images.activeImage);
  ...
  const { data: usersQuery, isSuccess: usersObtained } = useGetAllUsersQuery();
  useEffect(() => {
  ...
  const handleChangeFocusPic = (picture) => {
